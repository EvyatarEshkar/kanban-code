...
/// Centralized logging for Kanban — writes to ~/.kanban/logs/kanban.log.
/// Thread-safe, fire-and-forget. Use from anywhere in KanbanCore or Kanban.
public enum KanbanLog {
    ...
    private static let logPath: String = {
        (logDir as NSString).appendingPathComponent("kanban.log")
    }()

    private static let queue = DispatchQueue(label: "kanban.log", qos: .utility)

    /// Log a message with a subsystem tag.
    /// Example: `KanbanLog.info("reconciler", "Matched session \(id) to card \(cardId)")`
